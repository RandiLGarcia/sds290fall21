...
4.  Replot the scatterplot, but this time use `geom_jitter()`. What does `geom_jitter()` do? What was misleading about the initial scatterplot?
...
6.  Use residual plots, `plot(m_bty)`, to evaluate whether the conditions of least squares
    regression are reasonable. Comment on each of the plots generated.
...
As expected the relationship is quite strong---after all, the average score is calculated using the individual scores. We can actually look at the 
...
\[
  \begin{aligned}
\widehat{score} &= b_0 + b_1 \times bty\_avg + b_2 \times (0) \\
&= b_0 + b_1 \times bty\_avg\end{aligned}
\]
