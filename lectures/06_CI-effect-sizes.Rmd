...
```{r}
qplot(x = log(sd), y = log(m), data = ds) +
...
n_f = log(filter(ds, Color == "F")$n) #sample size for F
...
We are 95% confident that the true mean difference in (log) price between near colorless and colorless Diamonds is between -0.74 and 0.83.   
...
- Factor: `Bread`
- Response: `Ants`
- CI and effect size for difference between `MultiGrain` and `Rye`

**Dataset 2**: `Meniscus`

- Factor: `Method`
- Response: `Displacement`
- CI and effect size for difference between Method `1` and Method `2`
